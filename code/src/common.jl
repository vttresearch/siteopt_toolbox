...
"""
    add_unit_param2(c0, paramcols; directory = "")

    The function creates the standard format dataframe for unit object parameters.
    `c0::DataFrame` user input
    `paramcols::Vector{Symbol}` list of parameter columns
    `directory` path for possible time series files
"""
function add_unit_param2(c0, paramcols; directory = "")
    
    c1 = add_object_param(c0, :unit, paramcols; directory)
    insertcols!(c1, :Objectclass1 => "unit")
